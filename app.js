...
yargs.command({
    ...
    handler(argv) {                             // handles the input after submit
...
yargs.command({
    ...
    handler() {
       notes.listNotes()                // displays the list returned by the function
    }
})

// create read command
yargs.command({
    command: 'read',                    // command to use on the CLI to initiate
    describe: 'Read a note',            // description of what it does for the CLI help command
    builder: {
        title: {
            describe: 'Note title',
            demandOption: true,
    ...
    handler(argv) {
        notes.readNote(argv.title)      // returns the requested note
    }
})
